...
def create_user():
    ...
    if file_register == False:
        os.system("cls")
        user = input("Please enter your username to call in shell (no space): ")
        with open(userfile, "a") as write_sys:
            write_sys.writelines([user, "\n"])
        username_set()
...
def login():
    ...
    userinput = input("Please enter your username (no space): ")
...
def prompt():
    shell = input("user/" + str(user_print) + "/" + "main~$: ")
...
-------------------------------------------------
        ...
        Version: Dev-1.1 (Still in development!)
-------------------------------------------------
    """)
    choice()

def choice():
    check_choice = os.path.isfile(userfile)
    print("1) Create user")
...
def shutdownoptions():
    ...
    print("2) Restart")
    print("3) Back")
    shutdownchoice = input("Pick your choice: ")
    if shutdownchoice == "1":
        shutdown()
    if shutdownchoice == "2":
        restart()
